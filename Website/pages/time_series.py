...
def show_feature5():
    ...
    option = st.selectbox("Select Business Type", ("B2B", "C2C"))
        ...
        if st.button("Forecast"):
            ...
            # Load and preprocess data based on the selected option
                ...
                if option == "C2C":
                    ...
                    df_sales = preprocess_data(c2c_parquet)
                else:
                    b2c_parquet= "gs://spicy_1/customer_purchase*"
                    df_sales = preprocess_data(b2c_parquet)
...
def plot_aggregated_forecasts(df_sales, forecast):
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(weekly_sales_original['week'], weekly_sales_original['total_sales'], label='Original Sales', color='blue')
    ...
    # Plotting
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(monthly_sales_original['month'], monthly_sales_original['total_sales'], label='Original Sales', color='blue')
    for year in monthly_sales_forecast['year'].unique():
        year_data = monthly_sales_forecast[monthly_sales_forecast['year'] == year]
        ax.plot(year_data['month'], year_data['yhat'], label=f'Year {year}')
